...
def match_words(sentence, word_dict =None):
    """
    基于自定义词库进行中文分词匹配
    ...
    if not word_dict:
        word_dict = load_words_from_file()
...
def load_words_from_file(file_path=None):
    """
    从指定文件中加载词汇，每行一个词汇。如果 file_path 为空，默认读取 address.txt。
    ...
    """
    if not file_path:
        file_path = os.path.join(os.path.dirname(__file__), "address.txt")
...
if __name__ == "__main__":
    # 从文件加载词库
    # file_path = "address.txt"  # 文件路径
    # my_dict = load_words_from_file(file_path)
    # my_dict = load_words_from_file()
    ...
    matched = match_words(test_sentence)
